...
def find_latest_year_column(column_names: Iterable[str]) -> str:
    ...
    The latest survey-year column is extracted from column names from the DoBIH Munros
    and Tops CSV file.

    DoBIH classifies Munros and Tops in a single column per survey-year. Automatically
    finding the column of the latest survey-year allows for future releases of the CSV
    file to be handled appropriately, assuming the format of the file does not change.
